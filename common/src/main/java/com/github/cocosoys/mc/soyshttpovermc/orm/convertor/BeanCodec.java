...
public final class BeanCodec {
     ...
     */
    public static void serialize(Object bean, ConfigSection target) {
    ...
    private static Object encodeValue(Object value) {
        ...
        return value; // 标量直接存；List/Map/对象由 ConfigSection 原生支持（对象按 Map 存，需 map 化）
    }

    // ===== 反序列化：ConfigSection → Bean =====

    /**
     * 从 section 反序列化实体（字段缺失取默认值）。
     */
    public static <T> T deserialize(Class<T> beanClass, ConfigSection source) {
    ...
    private static Object decodeList(Object raw, Type genericType) {
        ...
        if (raw instanceof ConfigSection) {
            ConfigSection sec = (ConfigSection) raw;
            int i = 0;
            while (sec.get(String.valueOf(i)) != null) {
    ...
    private static Map<String, Object> decodeMap(Object raw) {
        ...
        if (raw instanceof ConfigSection) {
            for (String key : ((ConfigSection) raw).getKeys(false)) {
                out.put(key, ((ConfigSection) raw).get(key));
            }
        } else if (raw instanceof Map) {
    ...
    private static Object decodeNested(Object raw, Class<?> type) {
        ConfigSection sec = raw instanceof ConfigSection
                ? (ConfigSection) raw : yamlSectionOf(raw);
        return deserialize(type, sec);
    }

    private static ConfigSection yamlSectionOf(Object raw) {
        ConfigSection tmp = Platforms.getOrNull() == null ? null : Platforms.getOrNull().createYaml();
        if (tmp == null) return null;
        ConfigSection root = tmp.createSection("root");
