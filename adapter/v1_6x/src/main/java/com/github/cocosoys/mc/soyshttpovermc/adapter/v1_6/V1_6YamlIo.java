...
/**
 * 1.6.x 专用 YAML UTF-8 读写（v1_6x 模块内自实现，不与 v1_7x 共享）。
 *
 * <p><b>背景</b>：1.6.4 的 {@code YamlConfiguration.loadConfiguration(File)} 内部用
 ...
 * 且 1.6.4 比 1.7.10 更缺 {@code loadConfiguration(Reader)} / {@code save(Writer)} 这类显式编码入口
 * （javap 实证：仅有 loadConfiguration(File|InputStream) + loadFromString + saveToString）。
 * 因此本实现只走全版本均存在的 {@code loadFromString(String)} / {@code saveToString()}，
 * 文件字节显式按 UTF-8 解码 / 编码，从根上绕开平台默认编码。</p>
 *
 * <p><b>容错语义</b>（与 1.6.4 {@code YamlConfiguration.loadConfiguration(File)} 对齐）：
 * 文件不存在 / 读取或解析失败时返回空配置，不抛异常。</p>
 */
final class V1_6YamlIo {
