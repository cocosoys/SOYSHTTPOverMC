...
/**
 * API / 网页 / 导航项 自动生命周期监听器（从 {@code HttpOverMcPlugin} 抽离）：
 * 任一插件（非本插件）被禁用时，自动卸载其名下全部注解式 API、登记网页与门户导航项，
...
public class ApiLifecycleListener implements Listener {
    ...
    public ApiLifecycleListener(ApiRegistry apiRegistry, WebRegistry webRegistry, NavRegistry navRegistry, Plugin hostPlugin) {
    ...
    public void onPluginDisable(PluginDisableEvent e) {
        ...
        if (navRegistry != null) navRegistry.unregisterPlugin(name);
    }
}
