...
public class Query<T> {
    ...
    private soys.soyshttpovermc.orm.executor.IBackendExecutor executor;
    ...
    /** 门面注入执行器（默认 YAML）。 */
    public Query<T> setExecutor(soys.soyshttpovermc.orm.executor.IBackendExecutor executor) {
    ...
    public List<T> queryBeanList() {
        soys.soyshttpovermc.orm.executor.IBackendExecutor e = executor;
        if (e == null) {
            e = soys.soyshttpovermc.orm.YAML.Pojo.executor();
        }
