            ...
            Runnable connectTask = new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // [SOYS patch] 群组服转发兼容：host 形如 "ip\0ip\0uuid"（legacy IP 转发格式，
                        // NULL 字节 \0 分隔）时，TCP 连首个 \0 之前的部分（真实后端地址），
                        // 握手仍用完整 host（session.getHost() 返回完整值，由协议写入握手包）。
                        // 这样无头 Bot 在后端 bungee:true / Velocity legacy 转发下既能 TCP 连上后端，
                        // 又能在握手包里携带转发数据（host\0ip\0uuid）以通过 IP 转发校验、正常进服并注册通道。
                        String tcpHost = host;
                        int sep = tcpHost.indexOf('\u0000');
                        if (sep >= 0) tcpHost = tcpHost.substring(0, sep);
                        ...
                        try {
                            ...
                            String[] result = new InitialDirContext(environment).getAttributes(getPacketProtocol().getSRVRecordPrefix() + "._tcp." + tcpHost, new String[] {"SRV"}).get("srv").get().toString().split(" ", 4);
                            host = result[3];
