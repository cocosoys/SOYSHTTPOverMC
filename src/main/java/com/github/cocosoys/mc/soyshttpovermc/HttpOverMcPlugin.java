...
import com.github.cocosoys.mc.soyshttpovermc.web.WebContentCache;
...
import com.github.cocosoys.mc.soyshttpovermc.bot.BotGuardian;
...
public class HttpOverMcPlugin extends JavaPlugin {
    ...
    public void loadPagesConfig() {
        pagesConfig = ConfigManager.initPagesConfig(this);
    }
    ...
    public WebInterceptorRegistry getWebInterceptorRegistry() {
    ...
    public void onEnable() {
        ...
        eulaConfig = ConfigManager.initEulaConfig(this);
        if (!eulaConfig.isAccepted()) {
            EulaConfig.promptDisabled(getLogger());
        ...
        webInterceptorRegistry = new WebInterceptorRegistry();
        ...
        try {
            getServer().getPluginManager().callEvent(new SoysReadyEvent(api));
     ...
     * <p>复用 {@link YamlStorage#resolveDir} 的语义：
    ...
    private File resolveYamlOrmDir() {
        ...
        File dir = YamlStorage.resolveDir(this, fileCfg);
        if (!dir.exists()) {
            dir.mkdirs();
        }
        return dir;
    ...
    private void initWebCache() {
        try {
            ...
            largeFileLoaderRegistry = new LargeFileLoaderRegistry(largeThreshold);
            webContentCache = new WebContentCache(
    ...
    private void initStorage() {
        ...
        try {
            manager = new StorageManager(this);
            manager.initialize();
        } catch (Throwable t) {
        ...
        this.syncStorage = manager == null ? null : new RecordSyncStorage(manager);
    ...
    /**
     * 装配国际化环境（language.yml）：经 {@link ConfigManager#initLanguageConfig}
    ...
    private void initLanguageConfig() {
        languageConfig = ConfigManager.initLanguageConfig(this);
    }
    ...
    private void initApiFramework(File gatewayDir) {
        ...
        apiRegistry = new ApiRegistry(this);
